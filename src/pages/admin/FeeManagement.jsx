    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex dark-scrollbar">
      ...
      <main className="flex-1 p-8 overflow-y-auto max-w-7xl relative pb-28">
        ...
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8 pb-6 border-b border-slate-800/60">
          <div>
            <h1 className="text-2xl font-black text-white">Fee Ledger Sheet</h1>
            <p className="text-xs text-slate-400 font-light mt-1">Audit transactions, log cash entries and render receipt files.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-2.5 w-full lg:w-auto mt-4 lg:mt-0">
            <button
              ...
              className="bg-yellow-400 hover:bg-yellow-500 disabled:opacity-50 text-black border border-yellow-550 text-xs font-bold px-6 py-3.5 rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5 active:scale-95 cursor-pointer w-full sm:w-auto"
            >
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div>
            <select
              ...
              className="w-full bg-slate-900 border border-slate-800 focus:border-rose-500 rounded-xl px-4 py-2.5 text-slate-300 text-xs font-semibold cursor-pointer dark-input"
            >
                ...
                <tr className="border-b border-slate-800 text-slate-400 font-bold uppercase select-none">
                  <th className="py-3 px-3 w-10">
                    <input 
                      ...
                      className="rounded border-slate-800 bg-slate-950 text-rose-600 focus:ring-rose-605 h-3.5 w-3.5 cursor-pointer"
                    />
                  </th>
              ...
              <tbody className="divide-y divide-slate-850 font-semibold text-slate-300">
                {loading ? (
                  <tr>
                    <td colSpan={18} className="py-12 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-slate-500 text-xs font-light">Loading ledger matrix...</span>
                      </div>
                    </td>
                  </tr>
                ) : filteredStudents.length === 0 ? (
                  <tr>
                    <td colSpan={18} className="py-12 text-center text-slate-500 font-light">
                  ...
                  filteredStudents.map((stu) => {
                    ...
                    return (
                      <tr key={stu.id} className={`hover:bg-slate-800/30 transition-colors ${isChecked ? 'bg-indigo-950/20' : ''}`}>
                        ...
                        <td className="py-2.5 px-3">
                          <input 
                            ...
                            className="rounded border-slate-800 bg-slate-950 text-rose-650 focus:ring-rose-650 h-3.5 w-3.5 cursor-pointer"
                          />
                        </td>

                        {/* Name / Adm Code */}
                        <td className="py-2.5 px-3">
                          <div className="font-bold text-white text-sm truncate max-w-[140px]">{stu.fullName}</div>
                          <div className="text-[9px] text-slate-500 font-mono mt-0.5">{stu.admissionNumber || stu.studentId}</div>
                        </td>

                        {/* Class */}
                         <td className="py-2.5 px-3 text-center text-slate-400">{formatClassNameShort(stu.class)}</td>

                        {/* Month block columns */}
                        {ACADEMIC_MONTHS.map(m => {
                          const r = feeRecords[stu.studentId]?.[m];
                          if (!r) {
                            return <td key={m} className="py-2.5 px-1 text-center font-mono text-slate-600 text-[10px]">—</td>;
                          }
                            ...
                            <td key={m} className="py-2 px-1 text-center">
                              <button
                                ...
                                className={`w-full py-2 rounded-lg text-[9px] font-bold font-mono tracking-tighter border transition-all relative group flex flex-col items-center justify-center gap-0.5 cursor-pointer ${
                                  isPaid 
                                    ? 'bg-emerald-950/40 border-emerald-900/30 hover:bg-emerald-900/40 text-emerald-400' 
                                    : isPartial 
                                    ? 'bg-amber-955/40 border-amber-900/30 hover:bg-amber-900/40 text-amber-400' 
                                    : 'bg-rose-955/45 border border-rose-900/35 hover:bg-rose-900/40 text-rose-450'
                                }`}
                        ...
                        {/* Total Paid sum */}
                        <td className="py-2.5 px-3 text-right font-extrabold font-mono text-emerald-450">₹{paid}</td>

                        {/* Pending sum */}
                        <td className="py-2.5 px-3 text-right font-extrabold font-mono text-rose-455">₹{pending}</td>
        ...
        {editingCell && (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-6 z-50">
            <div className="bg-slate-900 border border-slate-800 max-w-md w-full rounded-3xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200 relative text-white">
              
              {/* Close Button */}
              <button 
                onClick={() => setEditingCell(null)}
                className="absolute top-4 right-4 p-2 text-slate-400 hover:text-white hover:bg-slate-850 rounded-lg transition-all cursor-pointer"
              >
                <X className="h-4.5 w-4.5" />
              </button>

              <header className="mb-6">
                <span className="text-[10px] text-rose-400 tracking-wider uppercase font-extrabold font-mono">Month: {editingCell.month}</span>
                <h3 className="text-lg font-black text-white mt-1 leading-tight">{editingCell.student.fullName}</h3>
                 <p className="text-[10px] text-slate-500 font-mono mt-0.5">ID: {editingCell.student.studentId} • {formatClassName(editingCell.student.class)}</p>
              </header>
              ...
              {modalActionType === 'pay' && (
                <div className="space-y-4">
                  <div className="bg-slate-950 border border-slate-850 p-4 rounded-xl flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-white font-mono">₹{editingCell.student.monthlyFee || 0}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      min="1"
                      className="w-full bg-slate-950 border border-slate-800 focus:border-rose-500 rounded-xl px-4 py-2.5 text-white font-mono text-sm focus:outline-none dark-input"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-950 border border-slate-800 focus:border-rose-500 rounded-xl px-4 py-2.5 text-slate-305 text-xs font-semibold cursor-pointer dark-input"
                    >
                  ...
                  <div className="flex gap-3 pt-4 border-t border-slate-800">
                    <button
                      ...
                      className="flex-1 bg-slate-950 border border-slate-800 hover:bg-slate-850 text-indigo-400 text-xs font-semibold py-2.5 rounded-xl transition-all flex justify-center items-center gap-1.5 cursor-pointer active:scale-95 disabled:opacity-50"
                    >
              ...
              {modalActionType === 'details' && (
                <div className="space-y-5">
                  <div className="bg-slate-950 border border-slate-850 p-4 rounded-xl space-y-2.5 text-xs font-light">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white font-bold font-mono">₹{editingCell.record.amount}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Transaction Mode</span>
                      <span className="text-slate-200 font-semibold">{editingCell.record.paymentMode || 'Online'}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between items-center pt-2.5 border-t border-slate-850">
                      ...
                      {editingCell.record.receiptId ? (
                        <span className="text-indigo-400 font-bold font-mono text-[10px] bg-indigo-950/40 border border-indigo-900/30 px-2 py-0.5 rounded shadow-sm">
                          {editingCell.record.receiptId}
                        </span>
                      ) : (
                        <span className="text-slate-500 text-[10px]">Not generated yet</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2.5">
                    ...
                    <button
                      ...
                      disabled={updatingRecord}
                      className="w-full bg-slate-950 border border-slate-800 hover:bg-slate-850 hover:border-slate-700 text-slate-300 text-xs font-bold py-2.5 rounded-xl transition-all flex justify-center items-center gap-1.5 disabled:opacity-50 cursor-pointer shadow-sm"
                    >
                      {updatingRecord ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileText className="h-4 w-4 text-rose-500" />}
                    ...
                    <button
                      ...
                      className="w-full bg-indigo-650 hover:bg-indigo-755 disabled:opacity-50 text-white text-xs font-bold py-2.5 rounded-xl transition-all flex justify-center items-center gap-1.5 cursor-pointer shadow-sm"
                    >
                  ...
                  {editingCell.record.whatsappSent && (
                    <div className="bg-emerald-950/20 border border-emerald-900/30 p-3.5 rounded-xl flex items-start gap-2.5 text-[10px] text-emerald-450 leading-normal font-light">
