    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex">
      ...
      <main className="flex-1 p-8 overflow-y-auto max-w-4xl">
        ...
        <button 
          ...
          className="flex items-center gap-2 text-slate-505 hover:text-slate-900 transition-colors mb-6 text-sm font-semibold group cursor-pointer"
        >
        ...
        <header className="mb-8 pb-6 border-b border-slate-200">
          <h1 className="text-2xl font-black text-slate-900">Enroll New Student</h1>
          <p className="text-xs text-slate-505 font-light mt-1">Add a new student profile manually to the database.</p>
        </header>

        {error && (
          <div className="mb-6 bg-rose-50 border border-rose-100 text-rose-700 p-4 rounded-xl text-xs font-light shadow-sm animate-pulse">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm space-y-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              type="button"
              onClick={addChild}
              className="text-xs font-semibold text-rose-600 hover:text-rose-700 transition-all flex items-center gap-1 cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-white border border-slate-200 focus:border-rose-500 rounded-xl px-4 py-2.5 text-slate-900 text-xs font-light transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-medium">+91</span>
                <input 
                  type="tel" 
                  pattern="[0-9]{10}"
                  maxLength="10"
                  ...
                  className="w-full bg-white border border-slate-200 focus:border-rose-500 rounded-xl pl-12 pr-4 py-2.5 text-slate-900 text-xs font-light transition-all"
                />
              </div>
            </div>
            ...
            {/* Location */}
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">City / Location *</label>
              <input 
                type="text" 
                value={location}
          ...
          <div className="space-y-6">
            {children.map((child, index) => (
              <div key={index} className="p-6 bg-white border border-slate-200 rounded-2xl relative space-y-6 shadow-sm">
                
                {children.length > 1 && (
                  <button
                    type="button"
                    onClick={() => removeChild(index)}
                    className="absolute top-4 right-4 text-slate-400 hover:text-rose-650 transition-colors flex items-center gap-1 text-[11px] font-semibold cursor-pointer"
                  >
                ...
                <h4 className="text-xs font-bold text-indigo-600 uppercase tracking-widest">Child #{index + 1} Profile</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Status Dropdown */}
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Admission Approval Status</label>
                    <select 
                      value={child.status}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-slate-100">
            <button
              type="button"
              onClick={() => navigate('/admin/students')}
              className="w-full sm:flex-1 bg-slate-50 border border-slate-200 hover:border-slate-355 text-slate-600 font-semibold py-3 px-5 rounded-xl text-xs transition-all active:scale-95 flex justify-center items-center cursor-pointer"
            >
              Discard
            </button>
            <button
